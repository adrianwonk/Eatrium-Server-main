    ...
    public String requestMade(Request request) {
        ...
        switch(cmd) {
            case CISConstants.PING:
                ...
                // println is used instead of System.out.println to print
                // to the server GUI
                println("   => " + PING_MSG);
                return PING_MSG;
            ...
            case "GET_USER_TYPE":
                try {
                    return "" + EatriumIDs.getIDType(request.getParam
                        (CISConstants.USER_ID_PARAM));
                }
                catch(Exception e){
                    return "N";
                }
            ...
            case CISConstants.CHECK_USER_EXIST:
                if (EatriumIDs.checkID(request.getParam
                (CISConstants.USER_ID_PARAM)))
                    return "true";
                else {
                    return "false";
    ...
    public String handleRequest(Request req) {
        ...
        if (user != null){
            if (acceptB){
                ...
                && EatriumIDs.getIDType(uID) == 'U')
                    return "successfully added user from requests";
                else {
                    return "unsuccessfully added user from requests";
                }
            }
            else {
                Menu.registerRequests.remove(user);
...
//              else handle all orders and minus balance
    ...
    public String placeOrder(Request req) {
        ...
        if (orderId == null || itemId == null || userId == null
        || orderType == null) {
            return CISConstants.PARAM_MISSING_ERR;
        }
        ...
        if (!userExists(userId)){
            ...
            try{
                ...
                else if (!menu.itemIDExists(itemId)){
                    return CISConstants.INVALID_MENU_ITEM_ERR;
                }
    ...
    public String addMenuItem(Request req) {
        ...
        double price = 0;
        if (!req.getParam(CISConstants.PRICE_PARAM).isEmpty())
        ...
        int amountAvail = 0;
        if (!req.getParam(CISConstants.AMOUNT_AVAIL_PARAM).isEmpty()) {
            amountAvail = Integer.parseInt(req.getParam(CISConstants
                .AMOUNT_AVAIL_PARAM));
        }

        if (itemName.isEmpty() || description.isEmpty() || type.isEmpty())
            return CISConstants.PARAM_MISSING_ERR;

        try {
            if (!(id == null || id.isEmpty())) {
                if (!menu.itemIDExists(id)){
                    MenuItem m = new MenuItem(itemName, description, price, id
                        , amountAvail, type);
                ...
                MenuItem m = new MenuItem(itemName, description, price
                    , amountAvail, type);
                menu.addEatriumItem(m);
            }
            return CISConstants.SUCCESS;
